...
exports.updateProfile = async (req, res, next) => {
  try {
    const allowedFields = [
      'bio', 'roles', 'playstyleTags', 'voiceChatPreference',
...
exports.findDuo = async (req, res, next) => {
  try {
    ...
    const [users, total] = await Promise.all([
      User.find(filter)
        .select('username avatar rank region roles playstyleTags voiceChatPreference bio isOnline lastSeen duoRating favoriteAgents riotId riotVerified')
